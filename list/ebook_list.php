...
<?php
    ...
    $query = "select * from realbook where Ebook= 'o'";
    ...
    $sql = "select * from realbook where Ebook= 'o' limit $start, $list_num;";
...
<div id="booklist" class="inner">
    ...
    if($page <= 1){
    ?>
        <a href="ebook_list.php?page=1">이전</a>
    <?php } else{ ?>
        <a href="ebook_list.php?page=<?php echo ($page-1); ?>">이전</a>
    <?php };?>

    <?php
        for($print_page = $s_pageNum ; $print_page <= $e_pageNum; $print_page ++){
    ?>
        <a href="ebook_list.php?page=<?=$print_page?>"><?=$print_page?></a>
    <?php
        }
    ?>
    <?php
        if($page >= $total_page){
    ?>
        <a href="ebook_list.php?page=<?=$total_page?>">다음</a>
    <?php
        } else {
    ?>
        <a href="ebook_list.php?page=<?=$page+1?>">다음</a>
    <?php
         }
    ?>
</div>
